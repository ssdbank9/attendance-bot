...
- `timein_bot.py`: production orchestration. Applies calendar/pause/duplicate guards, randomized timing, wake-capable one-shot registration, prior-day Time-Out catch-up, direct AKU API, headless Selenium fallback, retry, status recording, and notification. Calls `pull_from_cloud()` at startup to sync remote changes before decision logic. Checks `is_wfh()` and skips attendance on WFH days.
...
- `timein_history.json`: all successful attendance events exported from `attendance_db.py`. Git-tracked (not in `.gitignore`). Consumed by both `dashboard.py` and `docs/index.html` for analytics. Not pushed via `cloud_sync.py` Contents API; reaches GitHub through git commits.

### Calendar, leave, dashboard, and notifications

- `dashboard.py`: authenticated Flask application, local settings/credential management, attendance actions (using `Popen` + poll instead of `subprocess.run` with timeout), leave/holiday/blackout management, WFH management (add/cancel WFH dates and ranges), WFH hours clock card, attendance correction with correction email compose, analytics, pause sync, and API endpoints.
...
- `manage_blackout.py`: blackout/leave CLI maintenance. Supports WFH commands (`wfh`, `wfh-range`, `cancel-wfh`, `cancel-wfh-range`). WFH entries stored in `blackout.json` under `wfh` (single dates) and `wfh_ranges` (date ranges) arrays.
- `notify.py`: ntfy delivery with Claude CLI fallback and dashboard action links.
- `docs/index.html`: GitHub Pages phone dashboard (4-tab layout: Home, Leave, Analytics, Settings). Talks directly to GitHub Contents API and Actions API. Auto-refreshes every 60 seconds. Features: WFH clock card, attendance correction, correction email compose form, leave balance display, skip/leave/WFH management, analytics charts. Replaces `mobile_dashboard.html` for phone access.
- `mobile_dashboard.html`: legacy locally-served mobile dashboard. Superseded by `docs/index.html` for phone access; retained but not actively developed.

### Cloud and workflows

- `cloud_sync.py`: bidirectional GitHub Contents API sync. Push path: `blackout.json`, `holidays.json`, `leave_balance.json`, `timein_status.json`, `notification_prefs.json`, `bot_config.json`. Pull path (`pull_from_cloud()`): pulls `blackout.json`, `leave_balance.json`, `notification_prefs.json` from GitHub and merges with local (blackout gets date-level merge; others are remote-wins overwrite). `timein_bot.py` calls `pull_from_cloud()` at startup. Explicit refusal to sync AKU credentials.
- `leave_balance.json`: leave entitlements (annual, casual, sick, etc.). Bidirectionally synced via `cloud_sync.py`.
- `notification_prefs.json`: notification toggles and admin email address. Bidirectionally synced via `cloud_sync.py`.
...
7. `attendance.db` is the local source of truth. Do not make exported JSON authoritative and do not write event rows outside `attendance_db.record_event`. Exception: the phone dashboard (`docs/index.html`) can write attendance corrections directly to `timein_history.json` on GitHub; these are phone-initiated corrections that bypass the local DB.
...
12. WFH days cause the bot to skip attendance (`is_wfh()` in `timein_bot.py`). No system marks attendance on WFH days.
13. `timein_history.json` is generated from `attendance_db`, not manually edited on the desktop side. The phone dashboard writes corrections directly to GitHub's copy.
...
|---|---|---|
| Python syntax | `python -m py_compile attendance_db.py timein_bot.py dashboard.py set_pause.py cloud_deadman_check.py setup_new_user.py notify.py manage_blackout.py` | All changed Python files compile |
...
| WFH skip logic | Temporary blackout with WFH date matching today | `is_wfh()` returns True; `should_skip_today()` skips |
...
- Phone dashboard corrections write directly to `timein_history.json` on GitHub, bypassing the local `attendance_db`. A pull-and-reconcile mechanism to sync phone corrections back into the local DB does not yet exist.

## Feature backlog

1. Add committed unit tests for classifier allowlists, unmatched prior-day SQLite queries, pause freshness, installer abort behavior, and WFH skip logic.
...
5. Consolidate package requirements into a pinned, reviewed installer input without changing runtime dependencies casually.
6. Add a documented, authorized disaster-recovery exercise covering database restore, JSON regeneration, task re-registration, and deadman verification.
7. If the user later approves it, evaluate a fine-grained GitHub credential or GitHub App design for the Pages client.
8. Build a reconciliation mechanism to pull phone dashboard attendance corrections from GitHub back into the local `attendance_db`.
